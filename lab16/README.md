...
* Modify your Chirps table to require a relationship to a user (Every chirp must
have a user that sent it)
* Add a few users to the database
* In the Chirp form on the front-end, add an HTML select box (drop-down box) to
select the user for the chirp
* The box options should display the names of the users, but have values of the
ids of the users
